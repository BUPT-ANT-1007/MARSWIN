...
if __name__ == '__main__':
    # img = util.imread_uint('/root/autodl-tmp/lwk/train/synthesis_lq_x4/20220201161054_HX1-Or_GRAS_MoRIC-F-0012_SCI_N_20220201161054_20220201161054_00845_A.png', 3)
    # img = util.uint2single(img)
    sf = 4
    i = 0
    originPath='/root/autodl-tmp/lwk/train/clean'
    savePath='/root/autodl-tmp/lwk/train/synthesis_lq_x4_bsrgan_degradation_plus'
    for image_name in os.listdir(originPath):
        image_path = os.path.join(originPath,image_name)
        real_clean = util.imread_uint(image_path)
        img = util.uint2single(real_clean)
        img_lq, img_hq = degradation_bsrgan_plus(img, sf=sf, lq_patchsize=72)
        print(i)
        i = i + 1
        util.imsave(util.single2uint(img_lq), os.path.join(savePath,image_name))
